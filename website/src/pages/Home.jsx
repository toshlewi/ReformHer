...
function Feature({ title, desc, icon, delay = 0 }) {
  return (
    ...
    >
      ...
      <h4 className="font-semibold text-slate-800 group-hover:text-rose-700 transition-colors duration-300">{title}</h4>
...
function StepCard({ step, title, description, index }) {
  return (
    ...
    >
      <div className="text-rose-700 text-sm font-bold group-hover:scale-110 transition-transform duration-300">Step {step}</div>
...
export default function Home() {
  ...
  useEffect(() => {
    const sequence = async () => {
      await heroControls.start({ opacity: 1, y: 0 });
    };
    sequence();
  }, [heroControls]);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-rose-50 via-amber-50 to-white overflow-hidden">
        ...
        <div className="mt-6 grid md:grid-cols-2 gap-8 items-center">
          ...
          >
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900 leading-tight">
              Personalized micro-lessons for women — on <span className="text-rose-600">any</span> phone.
            </h2>
            <motion.p 
            ...
            >
              We deliver daily, bite-sized lessons via <b>USSD</b> &amp; <b>SMS</b> in{" "}
              <span className="underline decoration-amber-400 decoration-4">local languages</span> —
            ...
            <motion.p 
              ...
              viewport={{ once: true }}
            >
              Pilot dialing example: <b>*123#</b> (USSD) &bull; SMS opt-in: <b>JOIN</b>.
      ...
      <AnimatedSection id="how" className="max-w-6xl mx-auto px-5 py-12">
        <motion.h3 
          className="text-xl md:text-2xl font-bold text-slate-900 text-center"
          variants={fadeInUp}
        >
          How it works
        </motion.h3>
      ...
      {/* Features */}
      <AnimatedSection id="features" className="max-w-6xl mx-auto px-5 py-12 bg-rose-50/30 rounded-3xl mx-5 mb-8">
        <motion.h3 
          className="text-xl md:text-2xl font-bold text-slate-900 text-center"
          variants={fadeInUp}
        >
        ...
        <motion.p 
          className="text-slate-700 mt-2 text-center max-w-2xl mx-auto"
          variants={fadeInUp}
        >
      ...
      <AnimatedSection className="max-w-6xl mx-auto px-5 py-12">
        <div className="grid md:grid-cols-2 gap-8 items-center">
          ...
          >
            ...
            <motion.ul 
              className="mt-4 space-y-3 text-slate-700 text-sm"
              variants={staggerContainer}
            >
              {[
      ...
      >
        <div className="max-w-6xl mx-auto px-5 py-6 text-sm text-slate-600 flex flex-wrap items-center justify-between gap-3">
          ...
          <nav className="flex gap-4">
            <Link to="/privacy" className="hover:text-rose-700 transition-colors duration-300">Privacy</Link>
            <Link to="/contact" className="hover:text-rose-700 transition-colors duration-300">Contact</Link>
          </nav>
        </div>
      </motion.footer>
    </main>
